---
id: 0005
title: 'How to add dynamic routes to Next.js blog with Sanit.io content'
...
keywords:
  ...
  - 'JavaScript'
excerpt: 'In this post we look into how to fetch data from Sanity headless CMS and render dynamic pages using Next.js for a blog style site.'
image: static/images/how-to-add-dynamic-routes-to-next-js-blog-with-sanity-io-content/main-image.png
---

In the last post I wrote a short guide on [how to make a blog with Next.js and Sanity.io](https://sergiobarria.com/blog/how-to-make-a-blog-with-next-js-and-sanity-io). We cover almost all the basic needed to build a blog with **_Next.js_** and connect it to the headless CMS **_Sanity_**. The last piece needed to complete the puzzle was, how to add dynamic pages using the content we fetch from [Sanity](https://www.sanity.io/).

Adding dynamic pages is easier than you may think. So, without further to do, let's get to work.

## 1. Create a template for your routes

The first thing we need for our routes is a template. This template will be used to render all the pages that we need to look a same way. For example, let's say we are creating a blog, probably all our blog post will have the same look (a header, a body content and a footer is a basic example). So, for our blog posts we will create a template. Of course, the same example applies for any other content, a single product page for a e-commerce, a podcast page and so on.
...
**Note:** the place where you put the template file will determine what the final route will look like. If you put your template inside a folder called blog in your pages folder, then the final route will be `/blog/id.` This is standar Next.js behavior and its file-based routing system.
...
Remember that your template is like any other regular Next.js page, so you can use components here to. You can read more about dynamic routes and static generation [here](https://nextjs.org/docs/basic-features/pages). Also, in this component we are going to need our `sanity client config`. You can either create it here or in a separate file and then import it.

## 2. Fetching data from Sanity Studio
...
This will fetch all the slugs inside the content of type post. This query will return all the slugs that are in your **_Sanity Studio_** and **_Next_** will take care of generate one `.html` file for each **_slug_**.
...
Again we pass our sanity fetch function, this time using another parameters. First a postQuery, note here that I removed the query from the function again just to make it look cleaner. Here let's say I'm looking to get the _\_id, title, slug, image, published date and body content_ from Sanity studio.

```
// this is your [slug].js
...
The second parameter is the id for our single post, in this case, we use the slug. This will tell **_Next.js_** which specific content should be fetched for each `.html` page generated. The rest of the query are just examples of what you can query for. Our function then returns data that can be passed as props to our `[slug].js` component.

## 3. Pass the fetched data to our template

Now that we have our data, we can pass it as a prop to our template component (see the template code example on step 1).

Note that in this example, I'm passing the response as a `data` prop just because I named it the same way inside the `getStaticProps`. If you name it different you should use your variable name.
...
Luckly, Next got us cover. We can use [Incremental Static Regeneration](https://nextjs.org/docs/basic-features/data-fetching#incremental-static-regeneration) (ISR) which basically will rebuild our site after certain amount of time and fetch new data if there is any. This process will be automatic and will guarantee to always have our site up to date.
...
In this example, I'm telling **_Next.js_** to revalidate our data at most, every 600 seconds (10 minutes). The amount of time is up to you, it will depend on how up to date your content needs to be.

The other piece of the puzzle is, in our `getStaticPaths` change the `fallback` to true. Basically this will make that our pages that are not generate at build time (meaning the new content) will not result on a `404 page`.
...
This means that if our page is not ready yet or its been built, then we will only render a Loading message. Of course, you can get more creative here and render something like a loading spinner or anything really. We're using the `useRouter` hook and the `isFallback` method provided by Next in order to know if our route is ready or not.

## That's it!

That's it! everybody. Now we have all the basics we need to work with **_Next.js_** and **_Sanity.io_** together.
