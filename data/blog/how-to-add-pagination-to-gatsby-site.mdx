...
keywords:
  ...
  - 'pagination'
  - 'sanity io'
excerpt: 'In this tutorial, we learn how to add pagination to a Gatsby Blog that uses Sanity io headless CMS or any CMS'
image: '/static/images/blog/how-to-add-pagination-to-gatsby-site/post-cover.png'
---

Adding pagination is one of those things that a good part of the application we make will have, there for, is important to know how to add pagination to any page or component in our applications.
...
You can see on the code snippet below, we created a template page called `blog-list-template.js`. Inside this template the most important part to adding pagination is, inside our graphql query we need to add two (2) query variables (skip and limit). This variables help us determine how many items we'll have per page and how many items we have to skip in order to create the next page.

```js:blog-list-template.js
...
The first thing we have to get is the total amount of items in our data-collection. In my case, I'm retrieving the total amount of post stored in my `allSanityPost` Collection. To do so, we can use the totalCount parameter that graphql offer us.

**Note:** In the query I'm retrieving content from Sanity just because that's the CMS I'm using but here you can apply the same query to any CMS or wherever you have your content stored. Just change the collection name to the one you need and your good to go.

Next, we create all the variables we need to create our pagination. We have:

- `totalCount:` retrieved from our query. Used to get the amount of items in the collection.
- `postsPerPages:` you can call this variable however you like but here we are telling Gatsby how many items we want per page. In the example I'm using 10 posts per pages.
...
All this props are passed to our `template/page` via pageContext. For example, if your component needs the current-page variable you can extract it in the `template/page` component using `props.pageContext.currentPage` and then use it as any regular prop in your components.

The last thing to do is to export the `createPages` function and await for all the promises to resolve. Now just restart the server and check on your newly created pages. If you used the same path as in the example that means that your first page will be on the route `/blog` after that the next pages will be on `/blog/:index`.
